...
def prepare_val_given_dataset(config):
    # directory = './data/set/'
    # filename = 'match.csv'
    # matches = BadmintonCleaner(directory, filename)
    ...
    print("In prepare_val_given_dataset")
    print("haha",matches.head())
    # encode shot type

    codes_type_1, uniques_type_1 = pd.factorize(data['type'])
    CODE_TPYE = {}
    for i, val in enumerate(uniques_type_1):
        CODE_TPYE[val] = i
    codes_type=[]
    for i in matches['type']:
        codes_type.append(CODE_TPYE[i])
    #print(codes_type.shape) #edit
    #print(uniques_type_1) #edit
    #print(np.array(codes_type))
    matches['type'] = np.array(codes_type) + 1                                # Reserve code 0 for paddings
    config['uniques_type'] = uniques_type_1.to_list()
    #print(config['uniques_type']) #edit
    config['shot_num'] = len(uniques_type_1) + 1                      # Add padding
    #config['shot_num'] = 10 + 1                      # Add padding

    # encode player
    codes_player_1, uniques_player_1 = pd.factorize(data['player'])
    CODE_PLAYER = {}
    for i, val in enumerate(uniques_player_1):
        CODE_PLAYER[val] = i
    codes_player=[]
    for i in matches['player']:
        codes_player.append(CODE_PLAYER[i])

    matches['player'] = np.array(codes_player) + 1                            # Reserve code 0 for paddings
    config['uniques_player'] = uniques_player_1.to_list()
    config['player_num'] = len(uniques_player_1) + 1                  # Add padding
    #config['player_num'] = 35 + 1                  # Add padding
    #print("player_num",config['player_num'])
    config['folder_name'] = './model/'

    

    # use first 80% rallies in the match as train set, others as test set
    group = matches[['rally_id', 'ball_round', 'type', 'landing_x', 'landing_y', 'player', 'set','rally_length']].groupby('rally_id').apply(lambda r: (r['ball_round'].values, r['type'].values, r['landing_x'].values, r['landing_y'].values, r['player'].values, r['set'].values))
