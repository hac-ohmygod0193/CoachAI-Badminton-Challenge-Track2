...
def transformer_generator(given_seq, encoder, decoder, criterion, config, sample_id, device, ans_dict, rally_id, rally_length):#edit add rally_id parameter
    encode_length = config['encode_length']
    encoder.eval(), decoder.eval()
    mean_x, std_x = 172., 82.
    mean_y, std_y = 467., 192.
    #print("encode===",encode_length)
    loss = {
        'total': 1e6,
        'entropy': 1e6,
        'mse': 1e6,
    ...
    with torch.no_grad():
        ...
        for i in range(1):
            ...
            total_instance = rally_length-1 - len(given_seq['given_shot'][:encode_length])
            #print("instance",total_instance,len(given_seq['given_shot']),len(given_seq['given_shot'][:encode_length-1]) )
            #print("given shot",given_seq['given_shot'])
            
            for seq_idx in range(encode_length, rally_length-1): #edit +1
                ...
                target_player = given_seq['target_player'][(seq_idx+1)%2].unsqueeze(0).unsqueeze(0)
                ...
                #if sample_id == 1: print('output_shot_logits', output_shot_logits.shape) #edit
                ...
                #print(shot_prob[0])
                ans_dict['rally_id'].append(rally_id) #edit
                ans_dict['sample_id'].append(sample_id)
                ans_dict['ball_round'].append(seq_idx+2) #edit +1

                
                for i in range(10): #edit
                    ans_dict[config['uniques_type'][i]].append(shot_prob[0][seq_idx-encode_length][i+1].item())

                #if sample_id == 1: print('shot_prob', shot_prob.shape) #edit

                output_shot = shot_prob[0].multinomial(num_samples=1).unsqueeze(0)

                #if sample_id == 1: print('output_shot', output_shot.shape) #edit

                prev_shot = output_shot[:, -1, :]

                #if sample_id == 1: print('prev_shot', prev_shot.shape) #edit
                ...
                ans_dict['landing_x'].append(prev_x.item()) #edit
                ans_dict['landing_y'].append(prev_y.item()) #edit
    ...
    return loss, total_instance, generated_area, generated_shot, ans_dict #edit add ans_dict
