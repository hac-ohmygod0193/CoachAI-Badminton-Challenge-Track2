from badmintoncleaner import prepare_val_given_dataset
import ast
import sys
...
def draw(save_path, predict_area, predict_shot, true_area, true_shot):
    ...
    for i in range(len(predict_area)):
        ...
        type_english = predict_shot[i] #edit
        x.append(i_x)
        y.append(i_y)
        ...
        if i+1+5 == 8:
            #edit
            plt.annotate('{}: {}'.format(i+1+5, type_english), (i_x + 15, i_y - 20), rotation=90, fontsize=12)
            #plt.annotate('{}: {}'.format(i+1+5, shot_type_transform[type_chinese]), (i_x + 15, i_y - 20), rotation=90, fontsize=12)
        else:
            #edit
            plt.annotate('{}: {}'.format(i+1+5, type_english), (i_x + 5, i_y - 20), rotation=90, fontsize=12)
    ...
    for i in range(len(true_area)):
        ...
        i_y = true_area[i][1]
        i_shot = true_shot[i]
        type_english = config['uniques_type'][i_shot-1] #edit
        ...
        #edit
        if i+1 == 1:
            plt.annotate('{}: {}'.format(i+1, type_english), (i_x + 5, i_y - 20), rotation=90, fontsize=12)
        elif i+1 == 8:
            plt.annotate('{}: {}'.format(i+1, type_english), (i_x + 5, i_y - 50), rotation=90, fontsize=12)
        elif i+1 == 4:
            plt.annotate('{}: {}'.format(i+1, type_english), (i_x - 15, i_y - 60), rotation=90, fontsize=12)
        else:
            plt.annotate('{}: {}'.format(i+1, type_english), (i_x + 5, i_y - 40), rotation=90, fontsize=12)
...
print("Here",config['encode_length'])
set_seed(config['seed_value'])

# Prepare Dataset
config['filename'] = './data/val_given-1.csv' #edit
print(config['filename'])
matches, config = prepare_val_given_dataset(config)
#print(matches)
...
for sample_id in range(SAMPLES): #edit
    for rally_id in tqdm(matches['rally_id'].unique()):
        #print("sample_id",sample_id)
        #print("rally_id",rally_id)
    #for rally_id in [421]:
        #print(rally_id)
        selected_matches = matches.loc[(matches['rally_id'] == rally_id)][['rally_id', 'type', 'landing_x', 'landing_y', 'player','rally_length']].reset_index(drop=True)
        rally_length = selected_matches['rally_length'][0]
        #print(rally_length)
        #print("len matches",len(selected_matches),rally_id)
        # for case study
        ...
        #if len(selected_matches) < encode_length: #edit
        ...
        if len(selected_matches['player']) <= max_ball_round:
            given_seq = {
                ...
                'target_player': torch.tensor(selected_matches['player'][:].values).to(device)
            }
        else:
            given_seq = {
                'given_player': torch.tensor(selected_matches['player'][:].values).to(device),
        ...
        if config['model_type'] == 'LSTM':
            current_loss, total_instance, generated_area, generated_shot = GRU_generator(given_seq, encoder, decoder, criterion, config, SAMPLES, device)
        elif config['model_type'] == 'CFLSTM':
            current_loss, total_instance, generated_area, generated_shot = CFLSTM_generator(given_seq, encoder, decoder, criterion, config, SAMPLES, device)
        elif config['model_type'] == 'Transformer':
            current_loss, total_instance, generated_area, generated_shot,ans_dict = transformer_generator(given_seq, encoder, decoder, criterion, config, sample_id, device, ans_dict,rally_id,rally_length) #edit add ans_dict, rally_id change "SAMPLES"->sample_id
        elif config['model_type'] == 'DMA_Nets':                                                                                           #edit add ans_dict
            current_loss, total_instance, generated_area, generated_shot = DMA_Nets_generator(given_seq, encoder, decoder, criterion, config, SAMPLES, device)
        elif config['model_type'] == 'FuRaptor':
            current_loss, total_instance, generated_area, generated_shot, generated_decoder_attention, generated_decoder_encoder_attention, generated_decoder_encoder_disentangled = shotgen_generator(given_seq, encoder, decoder, criterion, config, SAMPLES, device)
        elif config['model_type'] == 'DNRI':
            current_loss, total_instance, generated_area, generated_shot = DNRI_generator(given_seq, encoder, decoder, criterion, config, SAMPLES, device)
        elif config['model_type'] == 'ShuttleNet':
            current_loss, total_instance, generated_area, generated_shot,ans_dict = shotgen_generator(given_seq, encoder, decoder, criterion, config, sample_id, device, ans_dict,rally_id,rally_length)
        else:
            raise NotImplementedError
    ...
    #edit add all codes below
#test
#edit add all codes below

df_final = pd.DataFrame()
for i in ans_dict:
    df_final[i] = ans_dict[i]
print(df_final.head())
...
# draw(save_path, generated_area, true_ar